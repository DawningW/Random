...
public class AutoCheck {
    ...
    public void check(MySynthesizer.Config initConfig, final Handler handler) {
    ...
    private AutoCheck innerCheck(MySynthesizer.Config config) {
        ...
        checks.put("检查AppId AppKey SecretKey",
                new AppInfoCheck(auth.getAppId(), auth.getAppKey(), auth.getSecretKey()));
        checks.put("检查包名", new ApplicationIdCheck(context, auth.getAppId()));

        if (!isOnlineSdk) {
            Map<String, String> params = config.getParams();
            String fileKey = SpeechSynthesizer.PARAM_TTS_TEXT_MODEL_FILE;
            checks.put("检查离线资源TEXT文件参数", new ParamKeyExistCheck(params, fileKey,
            ...
            checks.put("检查离线资源Speech文件参数", new ParamKeyExistCheck(params, fileKey,
    ...
    public String formatString(PrintConfig config) {
        ...
        for (HashMap.Entry<String, Check> entry : checks.entrySet()) {
            ...
            if (config.withLog && (config.withLogOnSuccess || hasError) && check.hasLog()) {
                sb.append("【日志】:" + check.getLogMessage()).append("\n");
            }
        }
        if (!hasError) {
