 ...
 * 语音合成引擎鉴权信息读取类
 * <p>
 * 详见 src/main/assets/auth.properties
 */
public class Auth {
    ...
    private Auth(Context context) {
        ...
        if (!context.getPackageName().equals(applicationId)) {
            throw new AuthCheckException("包名不一致，请在app/build.gradle 里修改 defaultConfig.applicationId。\n\n"
    ...
    private String getProperty(Properties properties, String key) {
        ...
        if (value == null) {
            throw new AuthCheckException("在 assets/auth.properties 里没有设置 " + key);
        }
        return value.trim();
    }
